        ...
        <div class="container">
            <div class="row">
                ...
                @foreach($favorites as $favorite)
                    <div>
                        <a href={{ route('profiles.show', $favorite->profile->id) }}><img src="{{ Storage::url($favorite->profile['image_path']) }}" alt="image" style="width: 25vw; height: auto;"/></a>
                    </div>
                    <div class="">
                        <h3>{{ $favorite->profile->name." ". $favorite->profile->age."歳" }}</h3>
                    </div>
                    <div style='font-size: 14em; '>
                      <button type="button" id="favorite-btn-{{ $favorite->profile->user_id}}" class="btn {{ $favorite->profile->favorite_profile()->where('user_id', Auth::id())->where('profile_id', $favorite->profile->user_id)->first() ? "bg-primary" : "bg-white"}}" 
